...
public class CPURepository : RepositoryService<CPU, HelperCPU>
{
    ...
    public override IReadOnlyCollection<CPU> SelectComponent(HelperCPU componentHelper)
    {
        if (componentHelper is null) return Enumerable.Empty<CPU>().ToImmutableList();
        ...
        {
            cpus = cpus.Where(cpu => cpu.SupportedMemoryFrequency is not null && cpu.SupportedMemoryFrequency.Intersect(componentHelper.SupportedMemoryFrequency).Any());
        }
