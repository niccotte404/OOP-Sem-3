...
namespace Itmo.ObjectOrientedProgramming.Lab2.Tests.WarningTests;
public class MainWarningTests
{
    [Fact]
    private void FirstTestCase()
    {
        ...
        Motherboard motherboard = repositoryMotherboard.SelectComponent(new HelperMotherboard { Socket = "LGA 1700" }).First();
        ...
        CPUCoolingSystem coolingSystem = repositoryCoolingSystem.SelectComponent(new HelperCoolingSystem { SupportedSockets = new List<string>() { "LGA 1700" } }).First();
        ...
        Assert.Equal(Results.Warning, result);
    }
    ...
    private void SecondTestCase()
    {
        ...
        CPU cpu = repositoryCPU.SelectComponent(new HelperCPU { Socket = motherboard.Socket }).First();
        BIOS bios = repositoryBIOS.SelectComponent(new HelperBIOS { SupportedCPU = new List<CPU>() { cpu } }).First();
        CPUCoolingSystem coolingSystem = repositoryCoolingSystem.SelectComponent(new HelperCoolingSystem { SupportedSockets = new List<string>() { "LGA 1700" }, MaxTDP = (float)(cpu.TDP * 0.9) }).First();
        XMPProfile xmp = context.XMPProfiles.First();
        GPU gpu = context.GPUs.First();
        SSD ssd = context.SSDs.First();
        HDD hdd = context.HDDs.First();
        ComputerCase computerCase = repositoryConputerCase.SelectComponent(new HelperComputerCase { MaxGPUFormFactor = gpu.FormFactor, MotherboardFormFactor = motherboard.FormFactor }).First();
        WiFiAdapter wifiAdapter = context.WiFiAdapters.First();
        float power = (float)((cpu.PowerConsumption + ram.PowerConsumption + gpu.PowerConsumption + ssd.PowerConsumprion + hdd.PowerConsumprion + wifiAdapter.PowerConsumption) * 1.3);
        PowerUnit powerUnit = repositoryPowerUnit.SelectComponent(new HelperPowerUnit { MaxPower = power }).First();
